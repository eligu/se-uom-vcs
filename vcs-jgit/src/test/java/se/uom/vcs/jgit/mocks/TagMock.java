...
import se.uom.vcs.jgit.VCSTagImp;

/**
 * Test mock for class {@link VCSTagImp}.<p>
 * 
 * Each tag mock has a one to one relation with {@link VCSTag} that is the fields are the same.
 * {@link #TAGS} array contains the tags in repository (see {@link RepoMock} and {@link #SMALL_REPO_TAG_NAMES}
 * contains only the names of each tag.
 * 
 * @author Elvis Ligu
 * 
 */
public class TagMock {
	...
	/**
	 * Tag names that are in the small repository.<p>
	 */
	...
	/**
	 * Some tags to be resolved that are in small repository.<p>
	 * <p>
	 */
	public static final TagMock[] TAGS = new TagMock[] {
			new TagMock("refs/tags/first_commit", "first_commit", CommitMock.COMMITS[CommitMock.INDEX_FIRST_COMMIT].id),
			new TagMock("refs/tags/fix_first_commit", "fix_first_commit", CommitMock.COMMITS[len-3].id)};
