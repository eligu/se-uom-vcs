...
public class VCSRepositoryImp implements VCSRepository {
	...
	/**
	 * The path where .git file will be stored.<p>
	 * This can be a clean path, when the remote path is specified
	 * and it will be cloned
	 */
	protected String localPath;

	/**
	 * The remote path (if any) from where this repository is cloned.<p>
	...
	/**
	 * The JGit repository object this repository is linked to.<p> 
	 */
	protected Repository repo;

	/**
	 * The selected branch from which we must get the head when parsing commits.<p>
	 * When repository is created the default selected branch will be MASTER (if any).
	 */
	protected VCSBranchImp selectedBranch;

	/**
	 * Creates a new instance given local and remote paths.<p>
	 * 
	 * <code>localPath</code> and <code>remotePath</code> can not be both null.
	 * 
	 * This will try to open a repository if the local path is not null and contains a .git
	 * directory. If this doesn't succeed it will throw an exception. Local path is not
	 * allowed to be null, however the remote path may be null if there is a repository
	 * under local path and this repository doesn't have a remote one.
	 * 
	 * @param 
	 * 		localPath where the repository is stored or will be stored in case of cloning.
	 * 		Can not be null.
	 * @param 
	 * 		remotePath the path from where the repository in <code>localPath</code> comes from
	 * 		or will be cloned.
	 * @throws 
	 * 		VCSRepositoryException in case the localPath contains a .git directory which can not be loaded
	 */
	public VCSRepositoryImp(String localPath, final String remotePath) throws VCSRepositoryException {
		...
		// If a .git directory is not contained in local path and a remote path is not specified
		// we can not create the instance because there will be no cloning, and no repo instance will
		// be loaded.
		if(!containsGitDir(localPath)) {

			ArgsCheck.notEmpty("no .git directory under local path, remotePath", localPath);
	...
	/**
	 * Open a repository from the path <code>local</code>.<p>
	 * The path must contain a .git directory or point to.
	 * 
	 * @param 
	 * 		local the path to load the repo from
	 * @return
	...
	static Repository openRepo(final String local) throws IOException, URISyntaxException {
		...
		final Repository repository = builder.setGitDir(gitDir)
	...
	/**
	 * Will correct the path and create a {@link File} instance that points to a .git directory.<p>
	...
	/**
	 * Check if path <code>local</code> points to an existing .git directory 
	 * or contains one.<p>
	 * 
	 * @param 
	 * 		local the path to check
	 * @return
	 */
	 ...
	 */
	@Override
	public void cloneRemote() throws VCSRepositoryException {
	...
	/**
	 * Check all branches and select the one that ends with /master or has the name master.<p>
	 * 
	 ...
	 * @throws VCSRepositoryException
	 */
	static void checkRepo(final Repository repo) throws VCSRepositoryException {
		if(repo == null) {
	...
	/**
	 * {@inheritDoc}
	 * <b>NOTE:</b> This will return all branches, local and remote ones if this repository
	 * is not been cloned, and there are local branches that follows remote ones.
	 */
	@Override
	public Collection<VCSBranch> getBranches() throws VCSRepositoryException {
		...
		try {
			...
			// Actually the repository will have only the remote branches (if this was cloned)
			// because we don't need to create a local branch.
			final List<VCSBranch> branches = new ArrayList<VCSBranch>();
			final List<Ref> refs = new Git(this.repo).branchList().setListMode(ListMode.ALL).call();
	 ...
	 */
	@Override
	public void update() throws VCSRepositoryException {
		...
		try {
			...
			// The source branches of remote repository (refs/heads/*) will be copied to remote
	 ...
	 */
	@Override
	public VCSCommit resolveCommit(final String cid) throws VCSRepositoryException {
		...
		try {
			...
			if(oid == null) {
				throw new VCSRepositoryException(cid + " can not be resolved to a commit");
			}
	 ...
	 */
	@Override
	public VCSBranch resolveBranch(final String bid) throws VCSRepositoryException {
		...
		try {
			...
			if(ref == null) {
				throw new VCSRepositoryException(bid +" can not be resolved to e reference");
			}
	...
	public VCSTag resolveTag(final String tag) throws VCSRepositoryException {
		...
		try {
			...
			if(ref == null) {
				throw new VCSRepositoryException(tag + " can not be resolved to e reference");
			}
	...
	public VCSCommit getFirst() throws VCSRepositoryException {
		...
		try {
			...
			final RevCommit root = walker.parseCommit(this.selectedBranch.branch.getObjectId());
	 ...
	 */
	@Override
	public void selectBranch(final VCSBranch branch) throws VCSRepositoryException {
		if(branch == null) {
			this.selectDefaultBranch();
		} else {
			this.selectedBranch = (VCSBranchImp) branch;
