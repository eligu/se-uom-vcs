...
import se.uom.vcs.jgit.walker.filter.commit.OptimizedCommitFilter;
...
public class VCSCommitImp implements VCSCommit {
     ...
     * <p>
     * 
     * <b>WARNING:</b> this method may fail if {@link #isFileMode(FileMode)}
     * returns false.
     * 
     * @param entry
     *            the diff entry
     ...
     * <p>
     * 
     * <b>WARNING:</b> this method may fail if {@link #isDirMode(FileMode)}
     * returns false.
     * 
     * @param entry
     *            the dif entry
	...
	final VCSCommitImp[] commits = checkCommitsForDiffAndReturnNewOld(
		commit, this);
	if (commits == null) {
	    return;
	}
	...
	if (resourceFilter == null && changeFilter == null) {
		...
		if (change != null) {
			...
			if (change.getType().isAdd()) {
			    include = resourceFilter.include(change
				    .getNewResource());
			    ...
			    include = resourceFilter.include(change
				    .getNewResource())
				    || resourceFilter.include(change
					    .getOldResource());
			}
		    }

		    if (changeFilter != null) {
			include = include && changeFilter.include(change);
		    }
     ...
     * @throws VCSRepositoryException
     */
    static boolean isAncestor(final RevCommit commit1, final RevCommit commit2,
     ...
     * @throws IOException
     */
    static RevCommit[] parentOf(final RevCommit child, final Repository repo)
	...
	try {
	    walker = TreeUtils.createTreeWalk(this.commit, this.repo, true,
		    (String[]) null);
		...
		if (isDirMode(mode)) {
		    ...
		    if (filter != null) {
			include = filter.include(dir);
			enter = filter.enter(dir);
		    }
		    ...
		    VCSFileImp file = new VCSFileImp(this,
			    walker.getPathString());
	...
	/*
	 * Here will walk the tree for the given commit. The tree walking will
	 * be recursive that is only the files will be written to disk, all
     ...
     */
    @Override
    public void walkCommitBack(final CommitVisitor<VCSCommit> visitor)
	    throws VCSRepositoryException {
	...
	try {
	    ...
	    VCSResourceFilter<VCSResource> resourceFilter = visitor.getResourceFilter();
	    if(resourceFilter != null) {
		OptimizedResourceFilter<VCSResource> of = ResourceFilter.parse(resourceFilter, null);
		if(of != null) {
