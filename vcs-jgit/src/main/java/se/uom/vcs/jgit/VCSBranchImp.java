...
/**
 * Implementation of {@link VCSBranch} based on JGit library.<p>
 * 
 * @author Elvis Ligu
 * @since 0.0.1
 * @version 0.0.1
...
public class VCSBranchImp implements VCSBranch {
	...
	/**
	 * The JGit branch reference this instance is related to.<p>
	 */
	protected Ref branch;

	/**
	 * The JGit repository reference this branch is related to.<p>
	 */
	protected Repository repo;

	/**
	 * Creates a new instance based on the given branch reference and repository.<p>
	 * 
	 * Branch reference and repository must not be null. Also branch reference must
	 * exists within repository. {@link Repository#getRef(String)} will be used
	 * to check if the given reference exists within repository.
	 * 
	 * @param 
	 * 		branch the reference this branch is related to 
	 * @param 
	 * 		repository the repository this branch comes from 
	 */
	...
	/**
	 * {@inheritDoc}
	 * {@link Repository#shortenRefName(String)} will be called to keep only a 
	 * human readable name of this branch.
	 */
	@Override
	 ...
	 */
	@Override
	public VCSCommit head() throws VCSRepositoryException{

		RevWalk walk = null;
		try {

			walk = new RevWalk(this.repo);
			return new VCSCommitImp(walk.parseCommit(this.branch.getObjectId()), this.repo);
	...
	public Collection<VCSCommit> getAllCommits() throws VCSRepositoryException {
		...
		this.walkCommits(new Visitor<VCSCommit>() {
	 ...
	 */
	@Override
	public void walkCommits(final Visitor<VCSCommit> visitor) throws VCSRepositoryException {
		this.head().walkCommitBack(visitor);
	}
	 ...
	 */
	@Override
	public boolean isContained(final VCSCommit commit) throws VCSRepositoryException {
		...
		try {
			return VCSCommitImp.isAncestor(rCommit, new RevWalk(this.repo).parseCommit(this.branch.getObjectId()), this.repo);
		} catch (final MissingObjectException e) {
			throw new VCSRepositoryException(e);
		} catch (final IncorrectObjectTypeException e) {
			throw new VCSRepositoryException(e);
