...
public class VCSDirectoryImp extends VCSResourceImp implements VCSDirectory {
	...
	if (filter != null) {
		...
		throw new IllegalArgumentException(
			"provided filter doesn't allow to enter path "
				+ this.path);
	    }
	}
	...
	try {
		...
		while (walk.next()) {
			...
			// This will ensure that we do not follow symlinks and
			// other submodules
			if (VCSCommitImp.isDirMode(walk.getFileMode(0))) {

			    // if directories will be included then add this
			    // entry
			    VCSResource resource = new VCSDirectoryImp(
				    this.commit, walk.getPathString());
			    if (dirs) {
				if (filter != null) {
				    ...
				    // Visitor is not willing to continue the
				    // tree
				    // walk
				    if (!visitor.visit(resource)) {
					return;
			...
			} else if (!visitor.visit(resource)) {
			    return;
			}
		    }
		}
