...
public class PathPrefixFilterTest {
	...
	VCSResourceMock rmock = new VCSResourceMock("src/path/test.java", VCSResource.Type.FILE, null);
	VCSResourceFilter<VCSResource> filter = filters.prefix("/src/path");
	assertTrue(filter.include(rmock));
	
	filter = filters.prefix("/src/path");
	...
	filter = filters.prefix("java/test/path", "java/path", "java");
	rmock.setPath("java");
	assertTrue(filter.include(rmock));
    }
	...
	rmock.setPath("java/test/test.txt"); // should be allowed because is a child of java/test that ends with .txt
	assertTrue(filter.include(rmock));
	
	rmock.setParent(new VCSResourceMock("java/main", null, null));
	rmock.setPath("java/main/file.java"); // should not be allowed because file.java is a child of java/main
	assertTrue(!filter.include(rmock));
	
	rmock.setParent(new VCSResourceMock("java/main/path", null, null));
	rmock.setPath("java/main/path/file.java"); // should be allowed because is under java/main but not a child of
	assertTrue(filter.include(rmock));
	
	rmock.setParent(new VCSResourceMock("src/main/java", null, null));
	rmock.setPath("src/main/java"); // should be allowed because is path src/main/java
	assertTrue(filter.include(rmock));
	
	rmock.setParent(new VCSResourceMock("src/java", null, null));
	rmock.setPath("src/java/test.java"); // should be allowed because is path src/java/test.java
	assertTrue(filter.include(rmock));
	
	rmock.setParent(new VCSResourceMock("java/path", null, null));
	rmock.setPath("java/path/test.txt"); // should be allowed because is a child of java/path that ends with .txt
	assertTrue(filter.include(rmock));
	
	rmock.setPath("java/path/test.java"); // should not be allowed because is a child of java/path that ends with .java
	assertTrue(!filter.include(rmock));
    }
	...
	rmock.setParent(new VCSResourceMock("java/main", null, null));
	rmock.setPath("java/main/test.java"); // should be allowed because is under the path java/main
	assertTrue(filter.enter(rmock));
	assertTrue(!filter.include(rmock)); // however should not be included because is a child of java/main
	
	rmock.setParent(new VCSResourceMock("java/main/path", null, null));
	rmock.setPath("java/main/path/file.java"); // should be allowed because is under java/main
	assertTrue(filter.enter(rmock));
	
	rmock.setPath("src/main/java"); // should be allowed because is path src/main/java
	assertTrue(!filter.enter(rmock));
	
	rmock.setPath("src/java"); // should be allowed because is path src/java/test.java
	assertTrue(filter.enter(rmock));
	
	rmock.setParent(new VCSResourceMock("java/main", null, null));
	rmock.setPath("java/main/p"); // should be allowed because is a child of java/path that ends with .java
	assertTrue(filter.enter(rmock));
    }
}
