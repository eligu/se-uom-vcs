...
/**
 * A filter that checks if a resource's path ends with a given suffix.<p>
 * 
 ...
 * @since 0.0.1
 */
public class SuffixFilter<T extends VCSResource>  extends ExactFilter<T> implements VCSResourceFilter<T> {

    /**
     * The set of suffixes to check each resource.<p>
     */
    private Set<String> suffixes;
    
    /**
     * Creates a new suffix filter based on given patterns.<p>
     * 
     * @param patterns must not be null, not or empty or contain any null element
     */
     ...
     * {@inheritDoc}
     * <p>
     * Returns true only if the path of the given resource ends with one of
     * the supplied suffixes.
     */
    @Override
