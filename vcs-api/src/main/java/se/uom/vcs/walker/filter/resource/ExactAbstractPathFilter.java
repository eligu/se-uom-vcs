...
/**
 * A base class for all path filters that require an exact path.<p>
 * 
 ...
 * the exact path should not be entered, that means we should stop the tree walking
 * at the exact path and collect the results if any. This allows us to make any
 * necessary optimizations. Sub classes of this class when combined (AND operator) with another
 * filter that doesn't deal with paths, such as {@link SuffixFilter} should stop the
 * entering of any resource at the exact paths.
 * 
 * @author Elvis Ligu
 * @version 0.0.1
 * @since 0.0.1
 */
public abstract class ExactAbstractPathFilter<T extends VCSResource> extends AbstractPathFilter<T>
