...
/**
 * Check whether all the specified filters return true.
 * <p>
 * 
 * @author Elvis Ligu
 * @since 0.0.1
 * @version 0.0.1
 */
public class VCSResourceAndFilter<T extends VCSResource> extends
      AbstractResourceFilter<T> implements VCSResourceFilter<T> {

   /**
    * The set of filters to combine.
    * <p>
    */
   ...
   public VCSResourceAndFilter(Collection<VCSResourceFilter<T>> filters) {
      ...
      Set<VCSResourceFilter<T>> tfilters = new LinkedHashSet<VCSResourceFilter<T>>(
            filters);
            ...
            for (VCSResourceFilter<T> f1 : tfilters) {
               if (f1 instanceof AbstractResourceFilter) {
                  if (af.excludesAnd((AbstractResourceFilter<T>) f1)) {
                     throw new IllegalArgumentException("filter " + af
                           + " excludes " + f1);
                  }
               }
            }
   ...
   public boolean equals(Object obj) {
      ...
      if (getClass() != obj.getClass())
         return false;
      VCSResourceAndFilter<?> other = (VCSResourceAndFilter<?>) obj;
      if (filters == null) {
