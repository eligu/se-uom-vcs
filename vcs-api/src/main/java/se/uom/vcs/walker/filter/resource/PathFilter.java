...
/**
 * Filters only the required paths.<p>
 * 
 * This filter can be used to allow a certain path or a group of paths.
 * Each time a {@link VCSResource} is tested it will check if the path is contained
 ...
 * <p>
 * <b>WARNING:</b> This should not be used for checking if a given resource is a sub
 * resource in one of the supplied paths. Instead it will return true only if the given
 * resource's path is exactly the same as one of the supplied paths. Use 
 * {@link PathPrefixFilter} filter in order to check if a given resource is a sub 
 * resource of a given path. 
 * 
 ...
 * @since 0.0.1
 */
public class PathFilter<T extends VCSResource> extends ExactAbstractPathFilter<T> {
     ...
     * {@inheritDoc}
     * <p>
     * Will allow all paths to enter, even those that are contained in this filter
     * because we do not exclude sub resources of these paths, however only paths
     * that are not specified within this filter will be allowed to visit.
     */
    @Override
