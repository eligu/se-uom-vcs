 ...
 * <p>
 * It is recommended that all implementations of {@link VCSResourceFilter}
 * subclass this, as it would allow for making any required optimizations,
 * such as defining if two path filters in an AND clause contains same filters.
 *  
 * @author Elvis Ligu
 * @version 0.0.1
 * @since 0.0.1
 */
public abstract class AbstractPathFilter<T extends VCSResource> extends AbstractResourceFilter<T> {

    /**
     * Set of abstract paths.<p>
     */
     ...
     * {@inheritDoc}
     * <p>
     * This method will return true if only one of the supplied paths is a
     * prefix of this resource. That will ensure that we don't accidentally
    ...
    /**
     * Care should be taken here to not accidentally prevent the tree
     * walking when a given resource is a prefix of one of the given paths. 
     * If the given resource is a prefix of one of the supplied paths than 
     * it should be allowed. If the given resource is a sub prefix of the 
     * supplied paths than it should be allowed too. 
     * <p>
     * Subclasses may override this in case a another strategy should be followed.
     * This method is called by {@link #enter(VCSResource)}.
     * 
     * @param resource
     * 		to check if this should be entered
     * @return
     * 		true if the given resource is a prefix or 
     * 		a sub path of the given paths.
     */
    ...
    /**
     * In case when a not filter of a specified path filter should be
     * created, then the "enter" strategy is not just a NOT(enter).<p>
     * 
     * Generally speaking any sub path of the given paths this filter
     * contains should not be entered. Subclasses may use this method
     * if it is appropriate (this method is provided as a general
     * strategy for "not entering" and is not used within this class)
     * or may provide one that is more suited to their case.
     * 
     * @param path
     * 		the path to check
     * @return
     * 		true if this not filter should enter the specified path
     */
    protected boolean notEnter(String path) {
	
	for (String prefix : paths) {
	    if (this.paths.contains(path) || isPrefix(prefix, path)) {
		return false;
	    }
	}
	return true;
    ...
    /**
     * Two filters are considered equals if they are of the same class,
     * and contains the same paths.<p>
     */
    @Override
