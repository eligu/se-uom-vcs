 ...
 * and manage the processors he can just a single processor, and that could be a
 * queue of processors, constructed outside of the object itself, so we can
...
public interface ProcessorQueue<T> extends Processor<T> {
    ...
    * if a given processor is already queued. However, depending in
    * implementations a queue may allow only distinct processors, and one a good
   ...
   /**
    * @return the processors number currently at this queue.
    */
